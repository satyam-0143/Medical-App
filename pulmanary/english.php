    ...
    if (isset($data["P_id"]) && (isset($data["first_time_score"]) || isset($data["second_time_score"]))) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (isset($data["first_time_score"])) {
                ...
                $sql = "UPDATE patient_info SET `Stques1`='$first_time_score' WHERE P_id='$id'";
                mysqli_query($conn, $sql);
            }
            if (isset($data["second_time_score"])) {
                $second_time_score = mysqli_real_escape_string($conn, $data["second_time_score"]);
                $sql = "UPDATE patient_info SET `Stques2`='$second_time_score' WHERE P_id='$id'";
                mysqli_query($conn, $sql);
            }
        } else {
            ...
            $sql = "INSERT INTO patient_info (P_id, `Stques1`, `Stques2`) VALUES ('$id', '$first_time_score', '$second_time_score')";
            mysqli_query($conn, $sql);
        }
